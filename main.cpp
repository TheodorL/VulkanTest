...
class HelloTriangleApplication {
 ...
 private:
  ...
  void initVulkan() {
    ...
    for (auto time : frameTime) {
      time = std::chrono::high_resolution_clock::now();
    }
  }

  void updateDeltaTime() {
    size_t prevFrame = currentFrame == 0 ? frameTime.size() - 1 : currentFrame - 1;
    if (MAX_FRAMES_IN_FLIGHT > 1) {
      frameTime[currentFrame] = std::chrono::high_resolution_clock::now();
      deltaFrameTime = std::chrono::duration<float, std::chrono::seconds::period>(frameTime[currentFrame] - frameTime[prevFrame]).count();
    } else {
      auto prevTime = frameTime[currentFrame];
      frameTime[currentFrame] = std::chrono::high_resolution_clock::now();
      deltaFrameTime = std::chrono::duration<float, std::chrono::seconds::period>(frameTime[currentFrame] - prevTime).count();
    }
  }
  ...
  void drawFrame() {
    ...
    updateDeltaTime();
    ts::Keys keys = window->getKeys();
    camera->keys.up = keys.up;
  ...
  void createDepthResources() {
    ...
    createImage(swapChainExtent.width, swapChainExtent.height, 1u, msaaSamples, depthFormat, VK_IMAGE_TILING_OPTIMAL,
                VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                depthImage, depthImageMemory);
    depthImageView = createImageView(depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, 1u);
    transitionImageLayout(depthImage, depthFormat, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, 1u);
  }
  ...
  void createRenderPass() {
    ...
    if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &renderPass) != VK_SUCCESS) {
      throw std::runtime_error("failet to create render pass!");
    }
  }

  void createRenderPassWithResolve() {
    VkAttachmentDescription colorAttachment{};
    colorAttachment.format = swapChainImageFormat;
  ...
  std::array<std::chrono::time_point<std::chrono::_V2::system_clock, std::chrono::_V2::system_clock::duration>, 2> frameTime;
  float deltaFrameTime = 1.0f;
